  ...
  return (
    <main className="flex-1 w-full bg-[#f9fafb] min-h-screen">
      <header className="bg-white border-b border-gray-100 px-6 py-3 sticky top-0 z-50 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <Link href="/" className="transition-opacity hover:opacity-80 group flex items-center">
            <span className="font-serif text-lg tracking-[0.2em] text-foreground leading-none mt-1">URBAN UMMATI</span>
          </Link>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={cn(
                ...
                isCopied ? "bg-green-50 border-green-200 text-green-600 shadow-sm" : "bg-white border-border text-foreground hover:bg-gray-50",
              )}
            >
              {isCopied ? (
                <>
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
              ...
              <div className="relative pt-2 px-2">
                <div className="absolute top-4.25 left-0 right-0 h-0.5 bg-gray-50 mx-8" />
                <div
                  className="absolute top-4.25 left-0 h-0.5 bg-primary transition-all duration-1000 mx-8"
                ...
                <div className="relative flex justify-between">
                  {steps.map((step) => (
                    <div key={step.label} className="flex flex-col items-center w-1/4">
                      ...
                      <span className={cn("mt-3 text-[10px] font-bold uppercase tracking-wider", step.done ? "text-gray-900" : "text-gray-400")}>{step.label}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="divide-y divide-gray-50">
                {order.items.map((item) => (
                  <div key={item.productId} className="px-5 py-4 flex items-center gap-4 hover:bg-gray-50/50 transition-colors">
                    <div className="h-12 w-12 rounded-lg bg-gray-50 border border-gray-100 shrink-0 overflow-hidden">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-xs"><span className="text-gray-500">Subtotal</span><span className="font-semibold text-gray-900">${order.subtotal.toFixed(2)}</span></div>
                <div className="flex justify-between text-xs"><span className="text-gray-500">Shipping</span><span className="font-semibold text-gray-900">{order.shippingCost === 0 ? "Complimentary" : `$${order.shippingCost.toFixed(2)}`}</span></div>
                <div className="flex justify-between text-xs"><span className="text-gray-500">Tax</span><span className="font-semibold text-gray-900">${order.tax.toFixed(2)}</span></div>
            ...
            <Link href="/products" className="group flex items-center justify-between bg-gray-900 p-5 rounded-2xl text-white transition-all hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-gray-900/10">
              <div>
      ...
      <footer className="max-w-6xl mx-auto px-6 py-12 text-center border-t border-gray-100">
        <p className="text-[9px] font-bold uppercase tracking-[0.4em] text-gray-300">Refined Urban Lifestyle &bull; Designed in Canada</p>
      </footer>
    </main>
  );
}